<?php
    ...
    $pdf->Cell(2,6,':',0,0);
    $pdf->Cell(63,6,$data['judul'],0,0);
    $pdf->Cell(32,6,'Tanggal',0,0);
    $pdf->Cell(2,6,':',0,0);
    $pdf->Cell(69,6,tanggal(date('Y-m-d', strtotime($data["tanggal"]))),0,1);
    
    // Baris 3: Mata Ujian (full width)
    $pdf->Cell(28,6,'Mata Ujian',0,0);
    $pdf->Cell(2,6,':',0,0);
    $pdf->Cell(166,6,$data['nama_mapel'],0,1);

    //Membuat header tabel
    $pdf->Cell(10,3,'',0,1);
    $pdf->SetFont('Arial','B',10);
    $pdf->Cell(8,6,'No',1,0,'C');
    $pdf->Cell(45,6,'NIP',1,0,'C');
    $pdf->Cell(73,6,'Nama',1,0,'C');
    ...
    while ($data = mysqli_fetch_array($hasil)){
        ...
        $pdf->Cell(8,6,$no,1,0,'C');
        $pdf->Cell(45,6,$data["nis"],1,0,'C');
        $pdf->Cell(73,6, $data['nama_siswa'],1,0);
